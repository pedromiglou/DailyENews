...
class RedisValueClass(MutexValue):
    ...
    def set(self, value):
        return REDIS.hset(self.__name, self._redis_key, value)

    def setnx(self, value):
        return REDIS.hsetnx(self.__name, self._redis_key, value)
...
class Counter(prometheus_client.Counter):
    ...
    def _multi_samples(self):
        for suffix in '_total', '_created':
            for labels, value in REDIS.hgetall(self._name + suffix).items():
                yield (suffix,
...
class Gauge(prometheus_client.Gauge):
    ...
    def _multi_samples(self):
        for labels, value in REDIS.hgetall(self._name).items():

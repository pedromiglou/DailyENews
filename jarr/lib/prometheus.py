...
class RedisValueClass(MutexValue):
    ...
    def inc(self, amount):
        return get_redis_master_conn().hincrbyfloat(
            self.__name, self._redis_key, amount)

    def set(self, value):
        return get_redis_master_conn().hset(self.__name, self._redis_key, value)

    def setnx(self, value):
        return get_redis_master_conn().hsetnx(self.__name, self._redis_key, value)
...
class Counter(prometheus_client.Counter):
    ...
    def _multi_samples(self):
        for suffix in '_total', '_created':
            for labels, value in get_redis_slave_conn().hgetall(self._name + suffix).items():
                yield (suffix,
...
class Gauge(prometheus_client.Gauge):
    ...
    def _multi_samples(self):
        for labels, value in get_redis_slave_conn().hgetall(self._name).items():
